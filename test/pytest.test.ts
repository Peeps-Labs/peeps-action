...
function workspace(): { root: string; suite: string } {
...
async function fakePeeps(): Promise<{ url: string; received: Received; close: () => void }> {
  const received: Received = { events: new Map(), completed: [], artifacts: [], auth: new Set() };
  ...
  const server = createServer(async (req, res) => {
    ...
    if (url.pathname.endsWith("/artifacts")) received.artifacts.push(url.searchParams.get("path") ?? "");
